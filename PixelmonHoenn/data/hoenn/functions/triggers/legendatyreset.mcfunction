#Runs during the Hall of Fame credits, resets legendary Pokemon the player has encountered but not caught.



#azelf
execute as @s[tag=!AzelfCatch,tag=AzelfEncounter] run tellraw @s {"text":"You can now rebattle Azelf!","italic":true,"color":"gray"}
execute as @s[tag=!AzelfCatch,tag=AzelfEncounter] run tag @s remove Dialogue273
execute as @s[tag=!AzelfCatch,tag=AzelfEncounter] run tag @s remove AzelfEncounter



#cobalion
execute as @s[tag=!CobalionCatch,tag=CobalionEncounter] run tellraw @s {"text":"You can now rebattle Cobalion!","italic":true,"color":"gray"}
execute as @s[tag=!CobalionCatch,tag=CobalionEncounter] run tag @s remove Dialogue234
execute as @s[tag=!CobalionCatch,tag=CobalionEncounter] run tag @s remove CobalionEncounter



#cresselia
execute as @s[tag=!CresseliaCatch,tag=CresseliaEncounter] run tellraw @s {"text":"You can now rebattle Cresselia!","italic":true,"color":"gray"}
execute as @s[tag=!CresseliaCatch,tag=CresseliaEncounter] run tag @s remove Dialogue268
execute as @s[tag=!CresseliaCatch,tag=CresseliaEncounter] run tag @s remove CresseliaEncounter



#deoxys
execute as @s[tag=!DeoxysCatch,tag=Dialogue207] run tellraw @s {"text":"You can now rebattle Deoxys!","italic":true,"color":"gray"}
execute as @s[tag=!DeoxysCatch,tag=Dialogue207] run tag @s remove Dialogue206
execute as @s[tag=!DeoxysCatch,tag=Dialogue207] run tag @s remove Dialogue208
execute as @s[tag=!DeoxysCatch,tag=Dialogue207] run tag @s remove SpacePrompt
execute as @s[tag=!DeoxysCatch,tag=Dialogue207] run tag @s remove Dialogue207



#dialga
execute as @s[tag=!DialgaCatch,tag=DialgaEncounter] run tellraw @s {"text":"You can now rebattle Dialga!","italic":true,"color":"gray"}
execute as @s[tag=!DialgaCatch,tag=DialgaEncounter] run tag @s remove Dialogue265
execute as @s[tag=!DialgaCatch,tag=DialgaEncounter] run tag @s remove DialgaEncounter



#entei
execute as @s[tag=!EnteiCatch,tag=EnteiEncounter] run tellraw @s {"text":"You can now rebattle Entei!","italic":true,"color":"gray"}
execute as @s[tag=!EnteiCatch,tag=EnteiEncounter] run tag @s remove Dialogue238
execute as @s[tag=!EnteiCatch,tag=EnteiEncounter] run tag @s remove EnteiEncounter



#giratina
execute as @s[tag=!GiratinaCatch,tag=GiratinaEncounter] run tellraw @s {"text":"You can now rebattle Giratina!","italic":true,"color":"gray"}
execute as @s[tag=!GiratinaCatch,tag=GiratinaEncounter] run tag @s remove Dialogue267
execute as @s[tag=!GiratinaCatch,tag=GiratinaEncounter] run tag @s remove GiratinaEncounter



#groudon
execute as @s[tag=!GroudonCatch,tag=Dialogue147] run tellraw @s {"text":"You can now rebattle Groudon!","italic":true,"color":"gray"}
execute as @s[tag=!GroudonCatch,tag=Dialogue147] run tag @s remove Dialogue147



#heatran
execute as @s[tag=!HeatranCatch,tag=Dialogue225] run tellraw @s {"text":"You can now rebattle Heatran!","italic":true,"color":"gray"}
execute as @s[tag=!HeatranCatch,tag=Dialogue225] run tag @s remove HeatranEncounter
execute as @s[tag=!HeatranCatch,tag=Dialogue225] run tag @s remove Dialogue225



#hooh
execute as @s[tag=!HoohCatch,tag=HoohEncounter] run tellraw @s {"text":"You can now rebattle Ho-oh!","italic":true,"color":"gray"}
execute as @s[tag=!HoohCatch,tag=HoohEncounter] run tag @s remove Dialogue219
execute as @s[tag=!HoohCatch,tag=HoohEncounter] run tag @s remove HoohEncounter



#kyogre
execute as @s[tag=!KyogreCatch,tag=Dialogue148] run tellraw @s {"text":"You can now rebattle Kyogre!","italic":true,"color":"gray"}
execute as @s[tag=!KyogreCatch,tag=Dialogue148] run tag @s remove Dialogue148



#kyurem
execute as @s[tag=!KyuremCatch,tag=KyuremEncounter] run tellraw @s {"text":"You can now rebattle Kyurem!","italic":true,"color":"gray"}
execute as @s[tag=!KyuremCatch,tag=KyuremEncounter] run tag @s remove Dialogue271
execute as @s[tag=!KyuremCatch,tag=KyuremEncounter] run tag @s remove KyuremEncounter



#landorus
execute as @s[tag=!LandorusCatch,tag=LandorusEncounter] run tellraw @s {"text":"You can now rebattle Landorus!","italic":true,"color":"gray"}
execute as @s[tag=!LandorusCatch,tag=LandorusEncounter] run tag @s remove Dialogue264
execute as @s[tag=!LandorusCatch,tag=LandorusEncounter] run tag @s remove LandorusEncounter



#latias
execute as @s[tag=!LatiasCatch,tag=LatiasEncounter] run tellraw @s {"text":"You can now rebattle Latias!","italic":true,"color":"gray"}
execute as @s[tag=!LatiasCatch,tag=LatiasEncounter] run tag @s remove LatiasEncounter



#latios
execute as @s[tag=!LatiosCatch,tag=LatiosEncounter] run tellraw @s {"text":"You can now rebattle Latios!","italic":true,"color":"gray"}
execute as @s[tag=!LatiosCatch,tag=LatiosEncounter] run tag @s remove LatiosEncounter



#lugia
execute as @s[tag=!LugiaCatch,tag=LugiaEncounter] run tellraw @s {"text":"You can now rebattle Lugia!","italic":true,"color":"gray"}
execute as @s[tag=!LugiaCatch,tag=LugiaEncounter] run tag @s remove Dialogue221
execute as @s[tag=!LugiaCatch,tag=LugiaEncounter] run tag @s remove LugiaEncounter



#Mesprit
execute as @s[tag=!MespritCatch,tag=MespritEncounter] run tellraw @s {"text":"You can now rebattle Mesprit!","italic":true,"color":"gray"}
execute as @s[tag=!MespritCatch,tag=MespritEncounter] run tag @s remove Dialogue274
execute as @s[tag=!MespritCatch,tag=MespritEncounter] run tag @s remove MespritEncounter



#palkia
execute as @s[tag=!PalkiaCatch,tag=PalkiaEncounter] run tellraw @s {"text":"You can now rebattle Palkia!","italic":true,"color":"gray"}
execute as @s[tag=!PalkiaCatch,tag=PalkiaEncounter] run tag @s remove Dialogue266
execute as @s[tag=!PalkiaCatch,tag=PalkiaEncounter] run tag @s remove PalkiaEncounter



#raikou
execute as @s[tag=!RaikouCatch,tag=RaikouEncounter] run tellraw @s {"text":"You can now rebattle Raikou!","italic":true,"color":"gray"}
execute as @s[tag=!RaikouCatch,tag=RaikouEncounter] run tag @s remove Dialogue237
execute as @s[tag=!RaikouCatch,tag=RaikouEncounter] run tag @s remove RaikouEncounter



#rayquaza
execute as @s[tag=!RayquazaCatch,tag=RayquazaEncounter] run tellraw @s {"text":"You can now rebattle Rayquaza!","italic":true,"color":"gray"}
execute as @s[tag=!RayquazaCatch,tag=RayquazaEncounter] run tag @s remove RayquazaEncounter



#regice
execute as @s[tag=!RegigigasCatch,tag=Dialogue232] run tellraw @s {"text":"You can now rebattle Regice!","italic":true,"color":"gray"}
execute as @s[tag=!RegigigasCatch,tag=Dialogue232] run tag @s remove RegiceEncounter
execute as @s[tag=!RegigigasCatch,tag=Dialogue232] run tag @s remove Dialogue232



#regigigas
execute as @s[tag=!RegigigasCatch,tag=Dialogue233] run tellraw @s {"text":"You can now rebattle Regigigas!","italic":true,"color":"gray"}
execute as @s[tag=!RegigigasCatch,tag=Dialogue233] run tag @s remove RegigigasEncounter
execute as @s[tag=!RegigigasCatch,tag=Dialogue233] run tag @s remove Dialogue233



#regirock
execute as @s[tag=!RegigigasCatch,tag=Dialogue230] run tellraw @s {"text":"You can now rebattle Regirock!","italic":true,"color":"gray"}
execute as @s[tag=!RegigigasCatch,tag=Dialogue230] run tag @s remove RegirockEncounter
execute as @s[tag=!RegigigasCatch,tag=Dialogue230] run tag @s remove Dialogue230



#registeel
execute as @s[tag=!RegigigasCatch,tag=Dialogue231] run tellraw @s {"text":"You can now rebattle Registeel!","italic":true,"color":"gray"}
execute as @s[tag=!RegigigasCatch,tag=Dialogue231] run tag @s remove RegisteelEncounter
execute as @s[tag=!RegigigasCatch,tag=Dialogue231] run tag @s remove Dialogue231



#reshiram
execute as @s[tag=!ReshiramCatch,tag=ReshiramEncounter] run tellraw @s {"text":"You can now rebattle Reshiram!","italic":true,"color":"gray"}
execute as @s[tag=!ReshiramCatch,tag=ReshiramEncounter] run tag @s remove Dialogue269
execute as @s[tag=!ReshiramCatch,tag=ReshiramEncounter] run tag @s remove ReshiramEncounter



#suicune
execute as @s[tag=!SuicuneCatch,tag=SuicuneEncounter] run tellraw @s {"text":"You can now rebattle Suicune!","italic":true,"color":"gray"}
execute as @s[tag=!SuicuneCatch,tag=SuicuneEncounter] run tag @s remove Dialogue239
execute as @s[tag=!SuicuneCatch,tag=SuicuneEncounter] run tag @s remove SuicuneEncounter



#terrakion
execute as @s[tag=!TerrakionCatch,tag=TerrakionEncounter] run tellraw @s {"text":"You can now rebattle Terrakion!","italic":true,"color":"gray"}
execute as @s[tag=!TerrakionCatch,tag=TerrakionEncounter] run tag @s remove Dialogue235
execute as @s[tag=!TerrakionCatch,tag=TerrakionEncounter] run tag @s remove TerrakionEncounter



#thundurus
execute as @s[tag=!ThundurusCatch,tag=ThundurusEncounter] run tellraw @s {"text":"You can now rebattle Thundurus!","italic":true,"color":"gray"}
execute as @s[tag=!ThundurusCatch,tag=ThundurusEncounter] run tag @s remove Dialogue241
execute as @s[tag=!ThundurusCatch,tag=ThundurusEncounter] run tag @s remove ThundurusEncounter



#tornadus
execute as @s[tag=!TornadusCatch,tag=TornadusEncounter] run tellraw @s {"text":"You can now rebattle Tornadus!","italic":true,"color":"gray"}
execute as @s[tag=!TornadusCatch,tag=TornadusEncounter] run tag @s remove Dialogue240
execute as @s[tag=!TornadusCatch,tag=TornadusEncounter] run tag @s remove TornadusEncounter



#uxie
execute as @s[tag=!UxieCatch,tag=UxieEncounter] run tellraw @s {"text":"You can now rebattle Uxie!","italic":true,"color":"gray"}
execute as @s[tag=!UxieCatch,tag=UxieEncounter] run tag @s remove Dialogue272
execute as @s[tag=!UxieCatch,tag=UxieEncounter] run tag @s remove UxieEncounter



#virizion
execute as @s[tag=!VirizionCatch,tag=VirizionEncounter] run tellraw @s {"text":"You can now rebattle Virizion!","italic":true,"color":"gray"}
execute as @s[tag=!VirizionCatch,tag=VirizionEncounter] run tag @s remove Dialogue236
execute as @s[tag=!VirizionCatch,tag=VirizionEncounter] run tag @s remove VirizionEncounter



#zekrom
execute as @s[tag=!ZekromCatch,tag=ZekromEncounter] run tellraw @s {"text":"You can now rebattle Zekrom!","italic":true,"color":"gray"}
execute as @s[tag=!ZekromCatch,tag=ZekromEncounter] run tag @s remove Dialogue270
execute as @s[tag=!ZekromCatch,tag=ZekromEncounter] run tag @s remove ZekromEncounter
















#
